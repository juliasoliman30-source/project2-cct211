...
def filter_data_by_status(data, status):
    """
    Returns a new list containing only the game entries that match the given status.
    """
    filtered_data = []
    for row in data:
         d_status = row.get("Status")
         if d_status == status:
              filtered_data.append(row)
    return filtered_data

def add_row(data, title, platform, status, hours, rating, imagePath=""):
    """
    Adds a new game entry to the data.
    """
    newId = generate_unique_id(data)
    new_entry = {"ID": newId, "Title": title, "Platform": platform, "Status": status, "Hours": hours, "Rating": rating, "ImagePath": imagePath}
    data.append(new_entry)
    write_data_to_csv(data)

def update_row(data, id, title, platform, status, hours, rating, imagePath=""):
    """
    Updates an existing entry in the data.
    """
    for row in data:
...
def delete_row(data, id):
    """
    Deletes an existing entry with the given id from the data.
    """
